...
class PixivPicSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url=url, callback=self.parse, cookies=cookies_dict)
    ...
    def parse(self, response):
        #确认访问主页
        # print(response.xpath('//h4/text()').extract_first())

        print(response.xpath('//h1[@class]/text()').extract_first())
        ...
        # 处理进入收藏逻辑
        next_url = response.xpath('//nav/a[@class="sc-a0b4b67e-1 eVqNkv"]/@href').extract_first()
        print(next_url)
        if next_url != None:
